...
import { RuntimeAwareHoverOptions, RuntimeAwareHoverProvider } from './languageService/runtimeAwareHoverProvider';
...
import { RuntimeInspectionService } from './runtimeInspection';
...
export abstract class LanguageServerBase implements LanguageServerInterface, Disposable {
        ...
        if (runtimeService?.isAvailable()) {
            ...
            return workspace.service.run(async (program) => {
                const provider = new RuntimeAwareHoverProvider(
                    ...
                    token,
                    runtimeService,
                    this.getRuntimeHoverOptions(),
                );
                return provider.getHover();
            }, token);
        }

        // Fall back to standard hover provider
        return workspace.service.run((program) => {
    ...
    /**
     * Gets the runtime inspection service for a workspace, if available.
     * Override this method in subclasses to provide workspace-specific services.
     */
    ...
    /**
     * Gets runtime hover options. Override in subclasses to customize.
     */
    protected getRuntimeHoverOptions(): Partial<RuntimeAwareHoverOptions> | undefined {
        return undefined;
    }
